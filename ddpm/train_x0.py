...
from training.loss_x0 import loss_batch_x0
#from diffusion.recons_x0 import debug_diffusion_trajectory_x0
...
def _load_split_npz(path):
    ...
    s1, s2 = d["s1"], d["s2"]
    mpf = d["mpf"]
    mpf = None if mpf.size == 0 else mpf
    dates = d["dates"]
    return s1, s2, mpf, dates


def load_dataloaders_from_npz(out_dir, batch_size, num_workers):
    splits = {}
    for split in ["train", "val", "test"]:
        s1, s2, _, _ = _load_split_npz(os.path.join(out_dir, f"{split}.npz"))
...
def main():
    ...
    parser.add_argument("--save_dir", default="./runs/x0")
    ...
    parser.add_argument("--wandb_proj", default="s1_to_s2_ddpm_x0")
    ...
    project_name=(wandb.run.name if wandb.run else "train_x0"),
    ...
    model = UNetCondMultiScale(
        ...
        ch_mults=(1, 2, 4),
        t_dim=128,
        out_activation=None,   # z-score space
    ).to(device)
    ...
    best_val = float("inf")
    best_path = os.path.join(args.save_dir, "best_model_x0.pth")
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            ...
            for batch in tqdm(train_loader, desc=f"[x0] Epoch {epoch}"):
                loss, _ = loss_batch_x0(
            ...
            print(f"[x0] Epoch {epoch} | train {train_loss:.4f} | val {val_loss:.4f}")
            ...
            #     debug_diffusion_trajectory_x0(
            #         model, val_loader, device, scheduler, epoch,
            #         save_dir=os.path.join(args.save_dir, "recons")
            #     )
    finally:
